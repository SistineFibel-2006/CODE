...
namespace sIsTiNeFiBeL {
	...
	bool check (int t) {
		...
		int maxs = -INF, mins = INF, maxd = -INF, mind = INF;
		...
		return (work(maxs, mins, 0) && work(maxd, mind, 0)) || (work(maxs, mins, 1) && work(maxd, mind, 1));
		// int time = max(maxs - mins, maxd - mind);
		// if(t == 1 || t == 2) {
		// 	debug(t); debug(maxs); debug(mins); debug(maxd); debug(mind);
		// 	debug(time);
...
/**/cin >> n >> m;
		g.assign(n, vl(m, 0)); dis.assign(n, vl(m, INF));
		queue<pll> q;
		int x;
		F(i, 0, n) {
			F(j, 0, m) {
