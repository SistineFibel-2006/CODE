...
inline void matMul(int X[M][M], int Y[M][M], int Z[M][M]) {
    static __int128 t[M][M];
    memset(t, 0, sizeof t);
    for (int i = 0; i < M; i++) {
        for (int k = 0; k < M; k++) if (X[i][k]) {
            for (int j = 0; j < M; j++) {
                t[i][j] += __int128(X[i][k]) * Y[k][j];
            }
        }
    }
    for (int i = 0; i < M; i++)
...
inline void matPow(int X[M][M], int e, int R[M][M]) {
    static int T[M][M];
    // R = I
    for (int i = 0; i < M; i++)
        for (int j = 0; j < M; j++)
...
/**/int n, k; cin >> n >> k;
    ...
    int invk = modexp(k);
    int sumd = 0;
    int P1[16]; std::fill(P1, P1 + 16, 0);
    for(int c : d) {
      sumd = (sumd + c) % MOD;
      P1[c] = (P1[c] + invk) % MOD;
    }
    int m1 = sumd * invk % MOD;
    if(n == 1)
      return cout << m1 << endl, void();
    ...
    F(r, 0, 16)
      for(auto c : op)
        for(int x : d){
          int rr;
          switch (c) {
            case '+': rr = (r + x) & 15; break;
            case '*': rr = (r * x) & 15; break;
            case '&': rr = (r & x); break;
            case '|': rr = (r | x) & 15; break;
            default: rr = (r ^ x) & 15; break;
          }
          A[1 + rr][1 + r] = (A[1 + rr][1 + r] + inv5 * invk) % MOD;
        }
    ...
    F(r, 0, 16){
      int s = 0;
      for(int x : d) s = (s + (r & x)) % MOD;
      C[r] = s * invk % MOD;
    }

    A[0][0] = (m1 + 3) % MOD * inv5 % MOD;
    A[0][17] = (3 * m1 % MOD) * inv5 % MOD;

    F(r, 0, 16)
      A[0][1 + r] = (MOD - 2 * C[r] % MOD) * inv5 % MOD;

    matPow(A, n - 1, B);

    int v1[M], vn[M];
    v1[0] = m1;
    F(r, 0, 16) v1[1 + r] = P1[r];
    v1[17] = 1;

    F(i, 0, M) {
      __int128 tt = 0;
      F(j, 0, M)
        tt += __int128(B[i][j]) * v1[j];
      vn[i] = (int)(tt % MOD);
    }
