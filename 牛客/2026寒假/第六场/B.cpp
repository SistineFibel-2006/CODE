...
#define el '\n'
	...
	const I MAXN = 1000005;
	ll invf[MAXN];
	ll fac[MAXN];

	ll inv(ll n) {return modpow(n, MODD - 2, MODD);}

	void init() {
		fac[0] = 1;
		invf[0] = 1;
		rep(i, 1, MAXN) fac[i] = (fac[i - 1] * i) % MODD;
		invf[MAXN - 1] = inv(fac[MAXN - 1]);
		rrep(i, 0, MAXN - 1)	invf[i] = (invf[i + 1] * (i + 1)) % MODD;
	}

	ll C(I n, I m) {
		if(m < 0 || m > n) return 0;
		return fac[n] * invf[m] % MODD * invf[n - m] % MODD;
	}
...
/**/INT(n,x,t);
  	I y = n - x;
  	auto wk = [&](I x, I y) -> ll {
  		if(x == 0) return y == 0 ? 1LL : 0LL;
  		if(x < y) return 0;
  		return C(x - 1, y - 1);
  	};
  	ll ans = 0;
  	ans = (ans + wk(x, (t+2)/2) * wk(y, (t+1)/2)) % MODD;
  	ans = (ans + wk(x, (t+1)/2) * wk(y, (t+2)/2)) % MODD;
