...
/**/INT(n, m, a, b);
  	...
  	while(!empty(pq)) { // dij
  		I d = pq.top().fi, u = pq.top().se; pq.pop();
  		dbg(d, u);
  		if(d > dis[u]) continue;
  		I r = u / m, c = u % m;
  		rep(i, 4) {
  			I nr = r + dx[i], nc = c + dy[i];
  			dbg(nr, nc);
  			if(nr >= 0 && nr < n && nc >= 0 && nc < m) if(chmin(dis[nr * m + nc], max(d + 1, ut[nr * m + nc]))) pq.push({max(d + 1, ut[nr * m + nc]), nr * m + nc});
  		}
  	}
  	I ans = 0;
  	each(pp, dis)  chmax(ans, pp);
  	out(ans);
