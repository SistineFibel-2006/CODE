...
/**/INT(n,m,x,d);
  	...
  	rrep(i, x) {
  		I nc = water[i];
  		while(po < n && c[po].fi > nc) {
  			I u = c[po].se;
  			vis[u] = 1;
  			if(d <= 1) cnt ++;
  			each(v, e[u]) if(vis[v]) if(dsu.leader(u) != dsu.leader(v)) {
  				if(dsu.size(dsu.leader(u)) >= d) cnt --;
  				if(dsu.size(dsu.leader(v)) >= d) cnt --;
  				int nr = dsu.merge(u, v);
  				if(dsu.size(nr) >= d) cnt ++;
  			}
  			po ++;
  		}
  		ans[i] = cnt;
