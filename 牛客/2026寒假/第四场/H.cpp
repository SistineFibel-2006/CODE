...
/**/INT(n,m,q);
  	...
  	auto ck = [&](I x, I y) ->bool {return x >= 1 && x <= n && y >= 1 && y <= m;};

  	auto wk = [&](I r, I c) -> ll {
  		ll sum = 0;
  		rep(i,-2,3) rep(j,-2,3) if(abs(i)+abs(j)<=2) if(ck(r+i,c+j)) sum += g[r+i][c+j];
  		return sum;
  	};

  	rep(i,1,n+1) rep(j,1,m+1) in(g[i][j]);
  	rep(i,1,n+1) rep(j,1,m+1) 
  		s[i][j] = wk(i, j), pq.push({s[i][j],i,j});
  	...
  	rep(q) {
  		INT(x,y,z); g[x][y] += z;
  		// all the point that (x,y) dis <= 2, need to update "score"
  		rep(i,-2,3) rep(j,-2,3) if(abs(i) + abs(j) <= 2) if(ck(x+i,y+j)) s[x+i][y+j] += z, pq.push({s[x+i][y+j], x+i, y+j});
  	
  		dbg(pq);
	  	while(1) {
	  		auto top = pq.top();
	  		if(top.v == s[top.x][top.y]) {
	  			out(top.x,top.y);
	  			break;
	  		}
	  		pq.pop();
