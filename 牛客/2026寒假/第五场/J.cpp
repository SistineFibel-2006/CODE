...
/**/VV(I,a,3,3);
  	set<I> s;
  	each(c,a) each(cc, c) s.is(cc);
  	if(sz(s) != 9) return(No());
  	rep(i,3) if(a[i][0] + a[i][1] + a[i][2] != 15) return(No());
  	rep(i,3) if(a[0][i] + a[1][i] + a[2][i] != 15) return(No());
  	if(a[0][0] + a[1][1] + a[2][2] != 15) return(No());
  	if(a[2][0] + a[1][1] + a[0][2] != 15) return(No());
  	Yes();
return;};
}
