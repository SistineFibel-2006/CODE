...
/**/LL(n,k); DBL(rw,cl);
  	...
  	auto ck = [&](double t) -> bool {
  		vec(Seg, seg, 0);
  		each(s, a) {
  			dbg(s.p, s.v);
  			double r = s.v * t;
  			if(r <= rw) continue; // 半径不达=高度
  			double le = s.p - sqrt(r*r-rw*rw);
  			double ri = s.p + sqrt(r*r-rw*rw);
  			seg.pb({le,ri});
  		}
  		dbg(seg.l, seg.r);
  		sor(seg);
  		...
  		double nowp = 0.0;
  		while(nowp < cl) {
  			double mr = -1e18; bool ok = 0;
  			while(id < sz(seg) && seg[id].l <= nowp) 
  				chmax(mr, seg[id].r), ok = 1, id ++;
  			if(!ok || mr <= nowp) return 0;
  			cnt ++, nowp = mr;
  		}
  		return cnt <= k;
  	};

  	double lo = 0.0, hi = 4000000000.0;
  	rep(i,100) {
