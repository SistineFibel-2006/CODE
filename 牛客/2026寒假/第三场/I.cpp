...
/**/
  	...
  	vec(I, ba, MAXM, 0); vec(I, msk, MAXM, 0);
  	rep(k, sz(id)) {
  		I idx = id[k], v = a[idx] ^ b[idx];
  		I m = 1 << k;
  		rrep(j,0,MAXM) {
  			if((v >> j) & 1) {
  				if(!ba[j]) {ba[j] = v, msk[j] = m; break;}
  				v ^= ba[j], m ^= msk[j];
  			}
  		}
  	}
  	...
  	I ff = 0;
  	rrep(j,0,MAXM) if((A >> j) & 1) {
  		if(!ba[j]) return(out(-1));
  		A ^= ba[j];
  		ff ^= msk[j];
  	}

  	v<I> ans = a;
  	rep(k,sz(id)) if((ff >> k) & 1) ans[id[k]] = b[id[k]];
