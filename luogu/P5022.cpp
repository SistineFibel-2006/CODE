...
/**/INT(N, M);
  	v<set<I>> e(N + 1);
  	rep(M) {
  		INT(x, y); e[x].is(y), e[y].is(x);
  	}

  	unordered_map<I, bool> oncircle;
 		auto findc = [&]() -> void {
 			vec(I, deg, N + 1, 0);
 			...
 			v<I> ans;
 			rep(i, 1, N + 1) if(deg[i] > 1) oncircle[i] = 1;
 		};
 		findc();
 		
 		dbg(oncircle);

 		auto findfirst = [&](auto &&findfirst, int u = 1, int fa = 0) -> int {
 			if(oncircle[u]) return u;
 			else each(v, e[u]) if(v != fa) return findfirst(findfirst, v, u);
 		};
