...
struct Matrix{
    ...
    template<size_t K> Matrix<T, N, K> operator*(const Matrix<T, M, K>& b){
        Matrix<T, N, K> c;
        for(size_t j = 0; j < M; j++) for(size_t i = 0; i < N; i++) for(size_t k = 0; k < K; k++) c[i][k] = (c[i][k] + (a[i][j] * b[j][k]) % m) % m;
        return c;
    }
...
void Mainsol() {
	LL(p, q, a1, a2, n); in(m);
	Matrix<ll, 1, 2> B;
	B[0][0] = a2 % m, B[0][1] = a1 % m;
	Matrix<ll, 2, 2> C;
	C[0][0] = p, C[1][0] = q, C[0][1] = 1, C[1][1] = 0;
	if(n == 2) out(a2);
	elif(n == 1) out(a1);
	else {
		auto ans = B * C.pow(n - 2);
		out(ans[0][0]);
	}
}
