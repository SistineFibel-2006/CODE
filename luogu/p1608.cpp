...
/**/INT(N, M);
    ...
    rep(M) {
      INT(x, y, w);
      if(edj.count({x, y})) chmin(edj[{x, y}], w);
      else edj[{x, y}] = w;
    }

    each(x, w, edj) e[x.fi].pb({x.se, w});

  	v<ll> ans(N + 1, 0);
  	ans[1] = 1;
  	auto dij = [&](int n, int s) -> v<I> {
  		v<I> dis(n + 1, INF);
  		dis[s] = 0;
  		pq<pii> pq;
  		pq.push({0, s});
  	...
  	auto dis = dij(N, 1);
    if(dis[N] == INF) out("No answer");
    else print(dis[N], ans[N]);
