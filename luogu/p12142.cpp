...
/**/INT(N);
  	...
  	vector<ll> fac(siz + 10, 0), inv(siz + 10, 0);
  	fac[0] = 1;
  	rep(i, 1, sz(fac)) fac[i] = fac[i - 1] * i % mod;
  	inv[sz(inv) - 1] = qpow(fac[sz(inv) - 1], mod - 2);
  	for(int i = sz(inv) - 2; i >= 0; i --) inv[i] = inv[i + 1] * (i + 1) % mod; 

  	vector<int> mp(5e5 + 10, 0);
  	rep(i, N) {
  		INT(x); mp[x] ++;
  	}
