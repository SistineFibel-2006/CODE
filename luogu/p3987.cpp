...
namespace sIsTiNeFiBeL {
  ...
  inline void Tempest_Flare__The_Wind_Splitting_Magic_Bullet() {
    ...
    vector<i64> bit(n + 1, 0);
    auto bit_add = [&](int idx, i64 delta) {
      for (int i = idx; i <= n; i += i & -i) bit[i] += delta;
    };
    auto bit_sum_prefix = [&](int idx)-> i64 {
      i64 s = 0;
      for (int i = idx; i > 0; i -= i & -i) s += bit[i];
      return s;
    };
    auto bit_range = [&](int l, int r)-> i64 {
      if (l > r) return 0;
      return bit_sum_prefix(r) - bit_sum_prefix(l - 1);
    };

    F(i, 1, n + 1) bit_add(i, a[i]);

    vector< set<int> > pos(MAXN + 1);
    for (int i = 1; i <= n; ++i) {
      if (a[i] >= 0 && a[i] <= MAXN) pos[a[i]].insert(i);
    }

    vector<char> possible(MAXN + 1, 0); 
    for (int i = 1; i <= n; ++i) {
      int ai = a[i];
      if (ai <= 0) continue;
      for (int d = 1; 1LL * d * d <= ai; ++d) {
        if (ai % d == 0) {
          possible[d] = 1;
          int other = ai / d;
          if (other <= MAXN) possible[other] = 1;
        }
      }
    }
    vector< vector<int> > muls(MAXN + 1);
    for (int d = 1; d <= MAXN; ++d) {
      for (int v = d; v <= MAXN; v += d) {
        if (possible[v]) muls[d].push_back(v);
      }
    }
      ...
      if (op == 1) {
        ...
        for (int v : vec) {
          ...
          while (it != s.end() && *it <= r) {
            ...
            s.erase(cur);
            int nv = v / x;
            if (nv >= 0 && nv <= MAXN) pos[nv].insert(id);
            bit_add(id, (i64)nv - (i64)v);
            a[id] = nv;
          }
        }
      } else {
        I(l); I(r);
        cout << bit_range(l, r) << el;
      }
    }
