...
void solve(){
  ...
  for(int i = 2; i <= n - 1; i ++) {
    yuan[i] = i;
    if(md[i - 1] == a[md[i]].first)
      md[i + 1] = a[md[i]].second;
    else if(md[i - 1] == a[md[i]].second)
      md[i + 1] = a[md[i]].first;
    else {
      cout << -1;
      return;
  ...
  vector<int > shun(n + 1, 0), ni(n + 1, 0);
  int ans = 0;
  for(int i = 1; i <= n; i ++) {
    shun[(n - yuan[i] + md[i]) % n] ++;
    ni[(n - yuan[n - yuan[i] + 1] + md[i]) % n] ++;
  }

  for(int i = 0; i < n; i ++)
    ans = max(ans, max(shun[i], ni[i]));
