...
/**/INT(n, m);
  	...
  	rep(i, 1, n + 1) sor(e[i]);
  	int s = 0, ide = 0, ode = 0, ck = 1;
  	rep(i, 1, n + 1) {
  		if(odeg[i] != ideg[i]) {
  			ck = 0;
  			if(odeg[i] - ideg[i] == 1) ode ++, s = i;
  			elif (ideg[i] - odeg[i] == 1) ide ++;
  			else return(No());
  		}
  	}
  	if(!((ide == 1 && ode == 1 && !ck) || (ide == 0 && ode == 0 && ck))) return(No());
  	...
  	auto dfs = [&](int s, auto&& self) -> void {
  		while(now[s] < sz(e[s])) self(e[s][now[s] ++], self);
  		ans.pb(s);
  	};
