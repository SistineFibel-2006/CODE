...
/**/INT(n, k);
    VEC(I, a, n); VEC(I, b, n);
    if(sum(a) <= k) return(out(0));

    rep(i, n) {
      if(a[i] > b[i]) a[i] = a[i] - b[i], b[i] = 0;
      elif(a[i] == b[i]) a[i] = 0, b[i] = 0;
      else b[i] = b[i] - a[i], a[i] = 0;
    }
    I Sum = sum(a);
    if(Sum == 0) return(out(1));
    dbg(a); dbg(b);

    auto ck = [&](int x) -> bool {
      queue<pii > q;//a[i]
      v<I> A = a; each(c, a) A.pb(c);
      v<I> B = b;
      v<I> vis(n * 2, 0);
      I S = Sum;
      rep(i, 0, n) {
        ...
        while(B[i] != 0) {
          if(empty(q)) break;
          if(q.front().fi <= B[i]) {
            S -= q.front().fi, B[i] -= q.front().fi;
            A[q.front().se] = 0;
            dbg(q.front().se, "!");
            q.pop();
          } else {
            S -= B[i], q.front().fi -= B[i], B[i] = 0;
            A[q.front().se] -= B[i];
            dbg(q.front().se, "?");
          } 
        }
        while(!empty(q)) {
          if(B[i] == 0 && q.front().fi == 0) q.pop();
          else break;
        }
        if(S <= k) return 1;
      }
