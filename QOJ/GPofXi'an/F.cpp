...
/**/INT(n);
    ...
    rep(i, n) {
      if(deg[i] > 0) {
        ...
        do {
          ...
          if(d[now] != d[nxt]) {
            ll tt = abs(a[now] - a[nxt]);
            if(chmin(time, tt)) firstu = now;
          }
          dbg(deg[now], deg[nxt], now, nxt, time);
          
          now = nxt;
        } while(now != i);

        if(firstu != -1)
          ans[firstu] = time, q.push(firstu);
      }
    }
    ...
    while(!empty(q)) {
      ...
      each(pre, e[now]) {
        if(ans[pre] == -1) {
          dbg(ans[now], pre, now, a[pre], a[now], d[pre], d[now]);
          
          if(d[pre] != d[now] && abs(a[pre] - a[now]) <= ans[now]) 
            ans[pre] = abs(a[pre] - a[now]);
          else 
            ans[pre] = abs(2 * a[now] + ans[now] * d[now] - 2 * a[pre]);
