...
/**/INT(n);
  	...
  	rep(i, 0, n + 1) {
  		ll nowe = 2 * a[i] - pree;
  		if(nowe < 0) return(Impossible());

  		if(i != n) {
  			e[i] = nowe;
  			if(e[i] <= 0 || e[i] % 2 != 0) return(Impossible());
  		} else {
  			if(nowe != 0) return(Impossible());
  		}
  		pree = nowe;
  	...
  	rep(Sum) {
  		...
  		if(pos > 0 && l[pos - 1] > 0) {
  			if(!(l[pos - 1] == 1 && r[pos - 1] == 0 && pos < n && r[pos] > 0)) {
  				ans += 'L'; l[pos - 1] --;
  				pos --;
  				ok = 1;
  			}
  		...
  		if(pos < n && r[pos] > 0) {
  			if(!(r[pos] == 1 && l[pos] == 0 && pos > 0 && l[pos - 1] > 0)) {
  				ans += 'R', r[pos] --;
  				pos ++;
  				ok = 1;
  			}
