...
namespace SistineFibel{
    ...
    template<class T> ll sz(const T& a){ return a.size(); }
...
void Mainsol() {
	...
	rep(msk, 1 << n1) {
		ll ww = 0, vv = 0;
		rep(i, n1) {
			if(msk >> i & 1) ww += w[i], vv += v[i];
		}
		if(ww <= W) lft.pb({ww, vv});  
	}

	rep(msk, 1 << n2) {
		ll ww = 0, vv = 0;
		rep(i, n2) {
			if(msk >> i & 1) ww += w[i + n1], vv += v[i + n1];
		}
		if(ww <= W) rit.pb({ww, vv});  
	}
	sor(lft), sor(rit);
	...
	ll mv = -1;
	each(x, y, lft) {
		if(chmax(mv, y)) L.pb({x, y});
	}
	mv = -1;
	each(x, y, rit) {
		if(chmax(mv, y)) R.pb({x, y});
	}
