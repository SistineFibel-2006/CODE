...
void Mainsol() {
	...
	if(k == 0) {
		...
		ll now = c;
		v<ll> prec;
		while(now <= a.back() + c) prec.pb(now), now += k;
		v<double> f(sz(prec));
		f.back() = 1.0 * sum / n - prec.back() - k;
		rrep(i, sz(f) - 1) {
			double ss = 0;
			auto fd = upper_bound(all(a), f[i + 1]) - begin(a);
			dbg(a[fd], f[i + 1], i);
			ss += pre[n] - pre[fd];
			ss += f[i + 1] * fd;
			// rep(j, n) ss += max((double)a[j], f[i + 1]);
			f[i] = -(double)prec[i] + 1.0 / n * ss;
		}
		if(sz(f) == 1) {
			double ss = 0;
			rep(j, n) ss += max((double)a[j], f[0]);
			f[0] = -(double)prec[0] + 1.0 / n * ss;
		}
		dbg(f);
		out(f[0]);
	}
