...
void Mainsol() {
	...
	::ll totb = 0, tots = 0, cnt = 0;
	::v<bool> sell(N, false);
	::set<I> skip;
	::v<pll> c;

	rep(i, N) {
		if(v[i] <= a) cnt ++, totb += v[i];
		else {
			c.pb({v[i], i});
			if(cnt > 0) cnt --, sell[i] = 1, tots += v[i];
			else skip.is(i);
		}
	} 
	...
	rep(i, M) {
		auto&&[nb, id] = b[i];
		dbg(nb, id);
		while(po < sz(c) && c[po].fi < nb) {
			I idd = c[po].se;
			po ++;

			if(!sell[idd]) skip.erase(idd);
			else {
				tots -= v[idd];
				sell[idd] = 0;

				auto it = skip.lower_bound(idd);
				if(it != end(skip)) {
					I nowid = * it;
					skip.erase(it);
					sell[nowid] = 1;
