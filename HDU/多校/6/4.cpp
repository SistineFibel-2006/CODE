...
namespace sIsTiNeFiBeL {
  long long summ(const vector<int>& v) {
    long long s = 0;
    for (int x : v) {
        s += x;
    }
        ...
        for (int j = k; j >= 1; j --) {
            if (ok[j - 1]) {
                ...
                for (int col = 0; col < m; col ++) {
                    now[col] = max(dp[j - 1][col], row[col]);
                }

                if (!ok[j] || summ(now) > summ(dp[j])) {
                    dp[j] = now;
                    ok[j] = true;
                }
            }
