...
/**/int n;
    ...
    if (!ok) {
        ...
        int cntlr;
        if (N % 2 == 0) {
            cntlr = ((N / 2) % MOD * ((N + 1) % MOD)) % MOD;
        } else {
            cntlr = ((N % MOD) * (((N + 1) / 2) % MOD)) % MOD;
        }

        int cntsum = (N + 1) % MOD;
        int cntLR = (cntsum * cntsum) % MOD;
        
        count = (cntlr * cntLR) % MOD;

    } else {
        int cntlr = (mini % MOD * (n - maxi + 1) % MOD) % MOD;
        
        int cntl = (minv + n + 1) % MOD;
        int cntr = (n - maxv + 1) % MOD;
        int cntLR = (cntl * cntr) % MOD;
