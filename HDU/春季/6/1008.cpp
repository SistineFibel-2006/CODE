...
int kruskal(I N, const v<E>&e, bool ctrl = 1) {
	v<E> se = e;
	sort(all(se), [ctrl](const E&a, const E&b) {
		if(ctrl) return a.w > b.w;
		else return a.w < b.w;
	});
...
/**/INT(N, M);
  	...
  	I minx = kruskal(N, e, 0);
  	I maxx = kruskal(N, e);

  	if(minx == -1 || maxx == -1) out(0);
  	else out(maxx - minx + 1);
