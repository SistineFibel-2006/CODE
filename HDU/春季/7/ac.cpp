...
struct UnionFind{
    ...
    ll root(ll a){ return data[a] == a ? a : data[a] = root(data[a]); }
...
struct mm {
  ...
  //F(a, b, x) : 有a个股票，现在是b天，之前x天买入（最大只计算到2,因为如果是2且前两天涨，那么就会麦）
  /*
    if(x == 2 && A[i - 1] >= A[i - 2]) F(a, b - 1, x) - A_i = F(a + 1, b, 0)
    F(a, b - 1, x) - A_i = F(a + 1, b, min{x + 1, 2})
    F(a, b - 1, x) + A_i = F(a - 1, b, 0)
  */
