...
void Mainsol() {
	...
	rep(i, 3) rep(j, 3) {
		if(sz(a) != 2) return out("Columbina Hyposelenia");
		if(vis[i][j]) continue;
		bool allx = 1, ally = 1;
		int dia = 0, dia2 = 0;
		int dia4 = 2;
		...
		if(allx || ally) return out(i + 1, j + 1);
		auto na = a;
		na.is({i, j});
		each(x, y, na) {
			if(x == dia) dia ++;
		...
		if((dia == 3 && dia2 == 3) || (dia == 3 && dia4 == -1)) return out(i + 1, j + 1);
	}
	out("Columbina Hyposelenia");
}
