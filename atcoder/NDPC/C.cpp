...
/**/INT(N);
  	STR(s1, s2, s3);
  	f[0][0][0][0] = 1;
  	rep(l, N) rep(i, sz(s1)) rep(j, sz(s2)) rep(k, sz(s3)) {
  		if(!f[l][i][j][k].x) continue;
  		for(auto c = 'a'; c <= 'z'; c ++) {
  			I ni = c == s1[i] ? i + 1 : i;
  			I nj = c == s2[j] ? j + 1 : j;
  			I nk = c == s3[k] ? k + 1 : k;
  			if(ni < sz(s1) && nj < sz(s2) && nk < sz(s3))
  				f[l + 1][ni][nj][nk] += f[l][i][j][k];
  		}
  	}
  	mm ans = 0;
  	rep(i, sz(s1)) rep(j, sz(s2)) rep(k, sz(s3))
  		ans += f[N][i][j][k];
  	out(ans.x);
