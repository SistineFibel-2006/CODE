...
struct Tree {
	...
	T f(T l, T r) { 
		...
		res.maxs = max({l.maxs, r.maxs, l.suf + r.pre});
		res.asum = l.asum + r.asum;
		return res;
	} // (any associative fn)
	vector<T> s; int n;
	Tree(int n = 0) : n(n) {
		siz = 1;
		while(siz < n) siz <<= 1;
		s.assign(2 * siz, {0, 0, 0, 0, 0});
	}
	void update(int pos, ll val) {
		pos += siz;
		ll v = val;
		val = (val == 0 ? -1 : val);
		s[pos] = {val, max(0LL, val), max(0LL, val), max(0LL, val), v};
		for (; pos > 1; ) {
			pos >>= 1;
...
/**/INT(N, Q);
  	...
  	rep(i, N) {
  		LL(a);
  		segt.update(i, a);
  		dbg(segt.s[i].sum, segt.s[i].pre, segt.s[i].suf, segt.s[i].maxs, segt.s[i].asum);
  	}
  	rep(Q) {
  		LL(i, v);
  		segt.update(i - 1, v);
  		  		dbg(segt.s[i].sum, segt.s[i].pre, segt.s[i].suf, segt.s[i].maxs, segt.s[i].asum);
  		ll ans1 = segt.s[1].asum;
  		ll ans = segt.s[1].maxs;
  		out(ans1 - ans);;
  	}
