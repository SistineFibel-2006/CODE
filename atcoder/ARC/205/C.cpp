...
/**/I(n);
  	...
  	F(i, 1, n + 1) {
  		cin >> s[i] >> t[i];
  		if(s[i] < t[i]){
  			ps.pb({s[i], 0, 1, s[i], t[i]});
  			ps.pb({t[i], 0, 2, s[i], t[i]});
  		}
  		else {
  			ps.pb({t[i], s[i], 0, 0, 0});
  		}
  		ev.pb({min(s[i], t[i]), 1, (int)i});
  		ev.pb({max(s[i], t[i]), -1, (int)i});
  		mp[s[i]] = mp[t[i]] = 0;
  	}
  	...
  	for(auto &p : ps) {
  		if(p.t == 0)
  			...
  			I l = mp[p.l], r = mp[p.r] - 1;
  			if(r > l && sum(r) - sum(l) > 0) {
  				ok = 1; break;
  			}
  		}
  	}
  	...
  	F(i, 1, n + 1) {
  		if(s[i] > t[i]) {
  			ps.pb({t[i], 0, 1, t[i], s[i]});
  			ps.pb({s[i], 0, 2, t[i], s[i]});
  		} else {
  			ps.pb({s[i], t[i], 0, 0, 0});
  		}
  	}
