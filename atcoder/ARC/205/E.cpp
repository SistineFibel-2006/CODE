...
static inline I modpow(I a, long long e) {
    if (e == 0) return 1;
    a %= MOD;
    I r = 1;
    while (e) {
  ...
  inline void Tempest_Flare__The_Wind_Splitting_Magic_Bullet() {
    ...
    unordered_map<I, vector<I>> pos;
    pos.reserve(min((size_t)n, (size_t)(1u<<20)));
    for (I i = 1; i <= n; i ++) pos[a[i]].pb(i);

    unordered_map<u64, I> pown;
    pown.reserve(1 << 20);
    ...
    F(k0, 1, n + 1) {
        ...
        while (true) {
            auto it = pos.find(s);
            if (it != pos.end()) {
                const vi &vec = it->second;
                I cnt = (I)(upper_bound(vec.begin(), vec.end(), k) - vec.begin());
                if (cnt > 0) {
                    if (s == 0) {
                        ans = 0;
                        ok = true;
                        ...
                        u64 key = ((u64)s << 32) | cnt;
                        auto itc = pown.find(key);
                        if (itc != pown.end()) {
                            ans = (ans * itc->second) % MOD;
                        } else {
                            I p = (I)modpow((i64)s, (i64)cnt);
                            pown.emplace(key, p);
                            ans = (ans * p) % MOD;
                        }
                    }
