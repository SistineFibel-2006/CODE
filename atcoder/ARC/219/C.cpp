...
/**/LL(H, W);
  	...
  	sor(D);
  	dbg(D);
  	v<v<pll>> e(N + 2); //e_0 left// e_1 right
  	ll lst = -1, lstx = 0, po = 2;
  	rep(i, N) {
  		ll y = D[i].fi, x = D[i].se;
  		ll ny, nx;
  		if(i != N - 1) ny = D[i + 1].fi, nx = D[i + 1].se;
  		if(y == lst) e[po].pb({po - 1, x - lstx}), e[po - 1].pb({po, x - lstx});
  		else {
  			if(y != lst && y != ny) {
  				e[po].pb({0, x - 1}), e[po].pb({1, N - x});
  				e[0].pb({po, x - 1}), e[1].pb({po, N - x});
  			} elif (y == ny) {
  				e[0].pb({po, x - 1}), e[po].pb({0, x - 1});
  			} else {
  				cerr << "fuck" << endl;
  				assert(1);
  				exit(0);
