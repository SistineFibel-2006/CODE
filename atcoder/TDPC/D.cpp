...
double f[110][50][50][50]{0.0};

void Mainsol() {
	INT(N);
	LL(D);
	if(D == 1) return out(1.0), void();
	ll A, B, C; A = B = C = 0;
 	while(D % 2 == 0) D /= 2, A ++;
	while(D % 3 == 0) D /= 3, B ++;
	while(D % 5 == 0) D /= 5, C ++;
	if(D != 1) return out(0.0), void();
	dbg(A, B, C);
	...
	rep(i, N) {
		...
		rep(a, A + 1) rep(b, B + 1) rep(c, C + 1) {
			dbg(a, b, c);
			// if(a + b + c == i) { // a, b, c是因子数，与掷骰子次数无关
				f[i + 1][a][b][c] += f[i][a][b][c] * p;
				f[i + 1][min(a + 1, A)][b][c] += f[i][a][b][c] * p;
				f[i + 1][a][min(b + 1, B)][c] += f[i][a][b][c] * p;
				f[i + 1][min(a + 2, A)][b][c] += f[i][a][b][c] * p;
				f[i + 1][a][b][min(c + 1, C)] += f[i][a][b][c] * p;
				f[i + 1][min(A, a + 1)][min(b + 1, B)][c] += f[i][a][b][c] * p;
			// }
		}
	}
	out(f[N][A][B][C]);
