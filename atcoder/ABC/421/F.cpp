...
/**/I(q);
  	...
  	vi inv(q + 1, -1), od;
  	auto work = [&](auto self, int v) ->void{
  		inv[v] = od.size();
  		od.pb(v);
  		reverse(g[v]);
  		for(auto c : g[v])
  	...
  	atcoder::segtree<i64, op, e> seg(q + 1);
  	F(i, 0, q) {
  		if(t[i] == 1)
  			seg.set(inv[i + 1], i + 1);
  		else {
  			I l = min(inv[x[i]], inv[y[i]]);
  			I r = max(inv[x[i]], inv[y[i]]);
  			cout << seg.prod(l + 1, r) << el;
  			l ++;
  			while(l < r) {
  				l = seg.max_right<f>(l);
  				if(l == r)
