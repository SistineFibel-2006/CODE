...
/**/LL(N);
    ...
    rep(N) {
      LL(l, r);
      ll al = l, ar = r, bl = N - r, br = N - l;

      auto add = [&](v<I> &vec, ll ql, ll qr) {
        if(ql <= qr) vec[max(0LL, ql)] ++, vec[min(N, qr + 1)] --;
      };

      add(cnta, al, ar); add(cntb, bl, br);
      add(cnt, max(al, bl), min(ar, br));
    }
    ...
    mm ans = 0;
    rep(i, 1, N) {
      ll aa = cnta[i] - cnt[i], bb = cntb[i] - cnt[i];
      ll non = N - (aa + bb + cnt[i]);
      if(non == 0 && aa <= i && bb <= (N - i))
        ans += C.C(cnt[i], i - aa);
