...
/**/I(n); I(k);
    auto a = getv<i64>(n , 1);
    vector<vi> e(n + 1);
    vector dp(n + 1, vector(k + 2, vl(3, -(1LL << 60))));
    ...
    auto dfs = [&](auto self, I u, I p) -> void{
      dp[u][1][0] = a[u]; dp[u][0][2] = 0;
      for(int v : e[u])
        if(v != p)
          self(self, v, u);
      for(int v : e[u]) if(v != p) {
        F_(i, k, -1) {
          Fd(j, 0, i) {
            dp[u][i][2] = max(dp[u][i][2], dp[u][i - j][1] + max(dp[v][j + 1][0], dp[v][j + 1][1]));
            dp[u][i][2] = max(dp[u][i][2], dp[u][i - j][2] + max({dp[v][j][0], dp[v][j][1], dp[v][j][2]}));
            dp[u][i][1] = max(dp[u][i][1], dp[u][i - j][0] + max(dp[v][j + 1][0], dp[v][j + 1][1]));
            dp[u][i][1] = max(dp[u][i][1], dp[u][i - j][1] + max({dp[v][j][0], dp[v][j][1], dp[v][j][2]}));
            dp[u][i][0] = max(dp[u][i][0], dp[u][i - j][0] + max({dp[v][j][0], dp[v][j][1], dp[v][j][2]}));            
          }
        }
      }
    };
