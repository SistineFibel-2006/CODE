...
/**/I h, w; cin >> h >> w;
  	...
  	vector<vector<std::array<I, 2>>> dist(h, vector<std::array<I, 2>>(w, {INF, INF}));
  	queue<tuple<I,I,I>> q;
  	dist[sx][sy][0] = 0;
  	q.emplace(sx,sy,0);

  	auto en = [&](I i, I j, I p) -> bool{
  		char c = g[i][j];
  		if(c == '#') return 0;
  		if(c == 'o') return p == 0;
  		if(c == 'x') return p == 1;
  	...
  	while(!q.empty()) {
  		...
  		F(k, 0, 4) {
  			...
  			if(!en(xi,xj,p))
  				continue;
  			int nowp = p ^ (g[xi][xj] == '?');
  			if(dist[xi][xj][nowp] > dnow + 1) {
  				dist[xi][xj][nowp] = dnow + 1;
  				q.emplace(xi,xj,nowp);
  			}
  		}
  	}
  	puts("-1");
