...
/**/INT(n);
  	...
  	v<pii> d1(n + 1), d2(n + 1), up(n + 1);

  	auto dfs1 = [&](auto&& self, I u, I p)->void {
  		d1[u] = {0, u};
  		d2[u] = {-1,-1};
  		each(v, g[u]) {
  			if(v == p) continue;
  			self(self,v,u);
  			pii np = {d1[v].fi+1, d1[v].se};
  			if(np > d1[u]) {
  				d2[u] = d1[u];
  				d1[u] = np;
  			} elif(np > d2[u]) {
  	...
  	auto dfs2 = [&](auto&& self,I u, I p)->void {
  		each(v,g[u]) {
  			if(v == p) continue;
  			pii pu = {up[u].fi+1,up[u].se};
  			pii ps = (d1[u].se == d1[v].se) ?
  			(make_pair)(d2[u].fi+1,d2[u].se)
  			:(make_pair)(d1[u].fi+1,d1[u].se);
  			up[v] = max(pu,ps);
  			self(self,v,u);
  		}
  	};
