...
struct SegTree {
    struct Node { int sum, assign; };
    vector<Node> t;
    int n;
    SegTree(int _n): n(_n), t(4*_n+4, {0, -1}) {}
    ...
    void push(int p, int l, int r){
        if(t[p].assign != -1){
            int v = t[p].assign, m = (l+r)/2;
            t[p<<1].assign = v;
            t[p<<1].sum = v * (m-l+1) % MOD;
            t[p<<1|1].assign = v;
            t[p<<1|1].sum = v * (r-m) % MOD;
            t[p].assign = -1;
        }
    }
    ...
    int query_sum(int p, int l, int r, int L, int R){
        ...
        push(p,l,r);
        int m=(l+r)/2;
        return ( query_sum(p<<1 ,l,m,L,R)
               + query_sum(p<<1|1,m+1,r,L,R) ) % MOD;
    }
    void collect(int p, int l, int r, vector<int>& res){
        if(l==r){
            res[l] = t[p].sum;
            return;
        }
...
/**/int n, m; cin >> n >> m;
		...
		F(i, 0, m) {
			int l, r; cin >> l >> r;
			int sum = st.query_sum(1, 1, n, l, r);
			int L = r - l + 1;
			int v = sum * inv[L] % MOD;
			st.update_assign(1, 1, n, l, r, v);
		} 
