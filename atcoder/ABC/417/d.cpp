...
/**/    int n; cin >> n;
    ...
    vector<std::array<int,501>> dp(n+2);
    for(int m = 0; m <= 500; m++) 
    	dp[n+1][m] = m;

    auto work = [&](int s, int mood) {
        int need = pre[s-1] + (mood - 500);
        int l = s, r = n, pos = n+1;
        while(l <= r) {
            int mid = (l + r) >> 1;
            if(pre[mid] >= need) { pos = mid; r = mid - 1; }
            else l = mid + 1;
        }
        if(pos == n+1) {
            int rem = mood - (pre[n] - pre[s-1]);
            return (rem < 0 ? 0 : rem);
        }
        int rem = mood - (pre[pos] - pre[s-1]);
        int m2 = (rem < 0 ? 0 : rem);
        return dp[pos+1][m2];
    };
