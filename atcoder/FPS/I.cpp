...
/*
    [x^K] pi_(i = 1) ^ N (1 + A_i x)
    => O(N ^ 2) 
    func(a,b) = pi_(i=a)^b (1 + A_i x)
    func(1, N) = func(1, floor(N/2)) * func(floor(N/2) + 1, N) 
    乘法结合律
    O(N log N log N)
*/
...
/**/INT(N, K);
    ...
    auto wk = [&](auto && self, const vector<mm> &A, int l, int r) {
        if(l == r) {
            return vector<mm>{1, A[l]};
        }
        int mid = (l + r) / 2;
        vector<mm> ll = self(self, A, l, mid);
        vector<mm> rr = self(self, A, mid + 1, r);
        return conv(ll, rr);
    };

    // cout << 1 << endl;
    vector<mm> ans = wk(wk, A, 0, N - 1);
    if(K < sz(ans)) out(ans[K].x);
    else out(0);
