...
namespace sIsTiNeFiBeL {
	...
	// const ll INF = 2e18;
	const I MAXT = 58;
	const string wm = "warong";
	const string am = "answer";
	ll len[26][MAXT + 1];
...
/**/INT(n,q); STR(s);
  	...
  	vv(ll, pre, MAXT + 1, n + 1);
  	rep(t, MAXT + 1) rep(i,n) {
  		ll cur = pre[t][i];
  		ll add = len[s[i]-'a'][t];
  		pre[t][i+1] = min(LINF, cur + add);
  	}

  	auto get = [&](char s, ll t, ll x) {
  		char cur = s;
  		while(t > 0) {
  			if(cur != 'w' && cur != 'a') return cur;
  			const string & m = cur == 'w' ? wm : am;
  			int t1 = min(t - 1, (ll) MAXT);
  			each(ch, m) {
  				ll chl = len[ch-'a'][t1];
  				if(x > chl)
  					x -= chl;
