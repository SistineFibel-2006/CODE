...
namespace sIsTiNeFiBeL {
	...
	struct BIT {
	  ...
	  ll query(ll l, ll r) {  // sum of A[l, r)
...
/**/INT(N);
  	VEC(I, A, N);
  	I maxx = 0;
  	each(c, A) chmax(maxx, c);
  	v<I> mp(maxx + 1, 0);
  	BIT bit(maxx + 1);
  	each(c, A) mp[c] ++, bit.add(c, 1);

  	auto ck = [&](int k) -> bool {
  		if(k > maxx + 1) return 0;
  		auto cnt = mp;
  		BIT now = bit;
  		rrep(i, 0, k) {
  			if(cnt[i] > 0) {cnt[i] --, now.add(i, -1); continue;}
  			if(2 * i + 1 > maxx) return 0;
  			if(now.query(2 * i + 1) == now.sum) return 0;
  			I v = now.kth(now.query(2 * i + 1));
  			cnt[v] --;
  			now.add(v, -1);
  		}
  		return 1;
