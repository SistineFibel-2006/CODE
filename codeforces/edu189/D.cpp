...
/**/LL(N, X);
  	...
  	auto count = [&](ll l, ll r, ll reminder) -> mm {
  		if(l > r) return 0;
  		ll tt = l + ((reminder - l % 4 + 4) % 4);
  		if(tt > r) return 0;
  		return mm(((r - tt) / 4 + 1));
  	};

  	// auto count = [](ll l, ll r, ll rem) -> ll {
  	// 	if(l > r) return 0;
  	// 	return ll((r - rem) / 4 - (l - 1 - rem) / 4);
  	// };
  	...
  	if(X - 1 >= 3) 
  		(ans += count(3, X - 1, 3) * count(X, N, 3));

  	if(X <= N && X >= 2) 
  		(ans += count(X, N, 1));

  	if(X - 1 >= 5)
  		(ans += count(5, X - 1, 1) * count(X, N, 1));
