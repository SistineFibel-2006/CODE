...
/**/int n, k; cin >> n >> k;
    ...
    bool ok = 1;
    usti p, vis;
    vis.reserve((cntS.size() + cntT.size()) * 2);
    for(auto &c : cntS)
        vis.in(c.fi);
    for(auto &c : cntT)
        vis.in(c.fi);
    p.reserve(vis.size() * 2);

    for(int r : vis) {
        if(p.count(r)) continue;
        p.in(r);
        int sr = cntS.count(r) ? cntS[r] : 0;
        int tr = cntT.count(r) ? cntT[r] : 0;
        if(r == 0) {
            if(sr != tr) {ok = 0; break;}
            continue;
        }
        ...
        if(!p.count(tt))
            p.in(tt);
        int sp = cntS.count(tt) ? cntS[tt] : 0;
        int tp = cntT.count(tt) ? cntT[tt] : 0;

        if(sr + sp != tr + tp) {ok = 0; break;}
    }
