...
/**/int n; cin >> n;
    string a, b; cin >> a >> b;
    vl pa(n + 1, 0), pb(n + 1, 0);
    F(i, 0, n) {
      pa[i + 1] = pa[i] + (a[i] - '0');
      pb[i + 1] = pb[i] + (b[i] - '0');
    }

    vl ca(n + 1, 0), cb(n + 1, 0);
    int sumca = 0, sumcb = 0;
    F(i, 1, n + 1) {
      ca[i] = i - pa[i];
      cb[i] = i - pb[i];
    ...
    int tot = n * sumca + n * sumcb;
    vl da(n + 1, 0), db(n + 1, 0);
    F(i, 1, n + 1) {
      da[i] = 2 * pa[i] - i;
      db[i] = i - 2 * pb[i];
    }
    vl sortdb;
    Fd(i, 1, n) sortdb.push_back(db[i]);
    sort(all(sortdb));

    vl sortdbpre(n + 1, 0);
    F(i, 0, n)
      sortdbpre[i + 1] = sortdb[i] + sortdbpre[i];

    int summ = 0;
    Fd(i, 1, n) {
      int v = da[i];
      int p = lower_bound(all(sortdb), v) - sortdb.begin();
      int cntt = n - p;
      int sumt = sortdbpre[n] - sortdbpre[p];
