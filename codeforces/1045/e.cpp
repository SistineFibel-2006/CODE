...
/**/int n; cin >> n;
    ...
    unordered_map<int, vector<int>> mp; mp.reserve(n * 2);
    Fd(i, 0, n) mp[pre[i]].pb(i);

    vi now = a;
    vector<char> vis(max(0LL, n - 1), 0);
    int prev = 0; bool ok = 1;
    ...
    for(int i = 0; i < n && ok; i ++) {
      ...
      auto low = std::lower_bound(all(vec), prev + 1);
      bool F = 0;
      for(auto j = low; j != vec.end(); j ++) {
        int pos = *j;
        int r = pos - 1;
        if(r < max(i, prev)) continue;
        for(int k = r - 1; k >= prev; k --) {
          if(!vis[k]) {
            now[k] ^= now[k + 1];
            vis[k] = 1;
          }
