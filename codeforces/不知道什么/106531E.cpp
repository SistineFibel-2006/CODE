...
using P = pll;
void Mainsol() {
	auto dis = [](P a, P b) -> ll {return abs(b.fi-a.fi) + abs(b.se-a.se);};
	P A, B;
	in(A, B);
	ll ans = dis(A, B);
	INT(M); rep(M) {
		LL(D, K);
		P AA[2] = {{A.fi, K * A.fi + D}, {(A.se - D) * K, A.se}};
		P BB[2] = {{B.fi, B.fi * K + D}, {K * (B.se - D), B.se}};
		each(a, AA) each(b, BB) {
			chmin(ans, dis(A, a) + abs(a.fi - b.fi) + dis(B, b));
			dbg(ans);
		}
	}
	out(ans);
